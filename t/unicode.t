...
sub ind {
    my $s = u( shift(@_) );
    return
      index( diff( \( u("שלום"), u("שלוש") ), { STYLE => "Table" } ),
        $s );
}

# TEST
ok( ( ind("ש") >= 0 ), "Output in unicode." );

{
    ...
    # TEST
    ok( ( ind("\\x{05e9}") >= 0 ), "Output not in unicode." );

    # TEST
    ok( ( ind("ש") < 0 ), "Output not in unicode - no unicode char found." );
}
